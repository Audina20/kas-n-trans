...
function addBulanPembayaran($data) {
	...
	if (mysqli_fetch_assoc($check_bulan)) {
		...
		foreach ($dataAnggota as $ds) {
		 	$anggota .= "('{$ds['id_anggota']}','{$id_bulan_pembayaran}','','','','')";
            $anggota .= ",";
		}
		$anggota = rtrim($anggota,",");
		$query_anggota = mysqli_query($conn, $anggota);
	  	return mysqli_affected_rows($conn);
	}
}
...
function tambahAnggotaUangKas($data) {
	global $conn;
	$id_anggota = htmlspecialchars($data['id_anggota']);
	$id_bulan_pembayaran = htmlspecialchars($data['id_bulan_pembayaran']);
	$query = mysqli_query($conn, "INSERT INTO uang_kas VALUES ('', '$id_anggota', '$id_bulan_pembayaran', '0', '0', '0', '0')");
  	return mysqli_affected_rows($conn);
}
