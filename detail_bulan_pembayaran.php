<?php 
  ...
  $anggota_baru = mysqli_query($conn, "SELECT * FROM anggota");
  /** WHERE id_anggota NOT IN (SELECT id_anggota FROM uang_kas) ORDER BY nama_anggota ASC"); **/
  ...
  if (isset($_POST['btnTambahAnggota'])) {
    if (tambahAnggotaUangKas($_POST) > 0) {
          ...
          <div class="modal-body">
            <div class="form-group">
              ...
              <select name="id_anggota" id="id_anggota" class="form-control">
                <?php foreach ($anggota_baru as $dsb): ?>
                  <option value="<?= $dsb['id_anggota']; ?>"><?= $dsb['nama_anggota']; ?></option>
                <?php endforeach ?>
              </select>
              <a href="anggota.php?toggle_modal=tambahAnggotaModal">Tidak ada nama anggota diatas? Tambahkan anggota disini!</a>
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-danger" data-dismiss="modal"><i class="fas fa-fw fa-times"></i> Close</button>
            <button type="submit" name="btnTambahAnggota" class="btn btn-primary"><i class="fas fa-fw fa-save"></i> Save</button>
          </div>
        </div>
      </form>
    </div>
